...
- Four top-level pages: `index.html`, `services.html`, `book.html`, `contact.html`. Each is a fully self-contained HTML document (no templating/includes) — shared markup (nav, footer, `<head>` boilerplate) is duplicated across all four files. When editing shared chrome (nav links, footer links, font imports), update all four pages in the same pass.
